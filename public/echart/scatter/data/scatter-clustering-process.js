...
let step = ecStat.clustering.hierarchicalKMeans(originalData, {
  clusterCount: 6,
  outputType: "single",
...
let colorAll = ["#bbb", "#37A2DA", "#e06343", "#37a354", "#b55dba", "#b5bd48", "#8378EA", "#96BFFF"];
...
function renderItemPoint(params, api) {
  ...
  let contentColor = colorAll[clusterIdx];
  return {
    type: "circle",
    x: coord[0],
    y: coord[1],
...
function makeStepOption(option, data, centroids) {
  ...
  let boundaryData = centroids ? [[centroids[newCluIdx][0], centroids[newCluIdx][1], Math.sqrt(maxDist)]] : [];
  option.options.push({
    series: [
      {
        type: "custom",
...
for (let i = 1, stepResult; !(stepResult = step.next()).isEnd; i++) {
  makeStepOption(option, echarts.util.clone(stepResult.data), echarts.util.clone(stepResult.centroids));
