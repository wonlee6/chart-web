...
function ___bfChartRun(obama_budget_2012) {
  myChart.hideLoading();
  ...
  function getLevelOption(mode: Mode): echarts.TreemapSeriesOption["levels"] {
    return [
      {
        color:
          mode === 2 ? ["#c23531", "#314656", "#61a0a8", "#dd8668", "#91c7ae", "#6e7074", "#61a0a8", "#bda29a", "#44525d", "#c4ccd3"] : undefined,
        colorMappingBy: "id",
        itemStyle: {
          borderWidth: 3,
          gapWidth: 3
  ...
  function getTooltipFormatter(mode: Mode) {
    ...
    return function (info: any) {
      ...
      amount2011 = isValidNumber(amount2011) ? echarts.format.addCommas(amount2011 * 1000) + "$" : "-";

      let perHousehold = value[3];
      perHousehold = isValidNumber(perHousehold) ? echarts.format.addCommas(+perHousehold.toFixed(4) * 1000) + "$" : "-";
    ...
    return {
      ...
      label: {
        ...
        formatter: function (params: any) {
          let arr = ["{name|" + params.name + "}", "{hr|}", "{budget|$ " + echarts.format.addCommas(params.value[0]) + "} {label|budget}"];
          mode !== 1 && arr.push("{household|$ " + echarts.format.addCommas(+params.value[3].toFixed(4) * 1000) + "} {label|per household}");
          return arr.join("\n");
        },
        rich: {
          budget: {
